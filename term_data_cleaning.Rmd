...
```{r message=FALSE, warning=FALSE}
lpmm <- feols( stroke ~ gender , data = df , vcov = 'hetero' )
df$predm <- predict( lpmm )

gw <- ggplot(data = df) +
  geom_point(aes(x = gender, y = predm), size = 1, shape = 16) +
  geom_line(aes(x = gender, y = predm),  size=0.7) +
